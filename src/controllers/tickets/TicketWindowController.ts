...
class TicketWindowController {
  ...
  public async store(req: Request, res: Response): Promise<Response> {
    ...
    if (ticket.ticketWindow !== undefined && ticket.sectorId !== undefined && ticket.mode && ['NORMAL', 'PRIORITARIO'].includes(ticket.mode)) {
      try {
        /* VERIFICA SE EXISTE UMA SENHA PARA CHAMAR */
        var sql = '';
        if (ticket.mode === 'NORMAL') {
          sql = 'select t.id, t.numero, t.prioritario, t.atendimento, s.nome from ticket t inner join setor_senha s on t.setor_id = s.id where t.atendimento = false and t.setor_id = $1 ORDER BY (prioritario = false) DESC, numero ASC limit 1';
        } else {
          sql = 'select t.id, t.numero, t.prioritario, t.atendimento, s.nome from ticket t inner join setor_senha s on t.setor_id = s.id where t.atendimento = false and t.setor_id = $1 ORDER BY (prioritario = true) DESC, numero ASC limit 1';
        }
