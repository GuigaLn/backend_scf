  ...
  public async store (req: Request, res: Response): Promise<Response> {
    ...
    if(sector.prioritary !== undefined || sector.sectorId !== undefined) {
      try {
        ...
        */
        var sql = "SELECT t.numero, s.nome FROM ticket t INNER JOIN setor s ON t.setor_id = s.id WHERE setor_id = $1 AND prioritario = $2 ORDER BY t.id DESC limit 1";
        const { rows } = await poolTickets.query(sql, [sector.sectorId, sector.prioritary]);
  
        if(rows.length === 0) {
          var sql = "SELECT s.nome FROM setor s WHERE id = $1 limit 1";
          ...
          */
          ...
          var numberCall: String = returning.rows[0].numero.toString().padStart(3, '0');
